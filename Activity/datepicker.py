...
class DatePicker(ft.UserControl):
    ...
    def _year_month_selectors(self, year, month, hide_ymhm = False):
        prev_year = ft.IconButton(icon=ft.icons.ARROW_BACK,icon_size=18, data=self.PREV_YEAR, on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY, height=self.CELL_SIZE,)
        next_year = ft.IconButton(icon=ft.icons.ARROW_FORWARD,icon_size=18, data=self.NEXT_YEAR, on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY)
        prev_month = ft.IconButton(icon=ft.icons.ARROW_BACK,icon_size=18, data=self.PREV_MONTH, on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY)
        next_month = ft.IconButton(icon=ft.icons.ARROW_FORWARD,icon_size=18, data=self.NEXT_MONTH, on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY)
        ym = ft.Column([
                    ft.Row([
                        prev_year,
                        ft.Text(year),
    ...
    def _hour_minute_selector(self, hour, minute):
        hm = ft.Container(
            ...
            [
                ft.Row([
                    ft.IconButton(icon=ft.icons.ARROW_BACK,icon_size=18, data=self.PREV_HOUR, on_click=self._adjust_hh_min),
                    ft.Text(hour),
                    ft.IconButton(icon=ft.icons.ARROW_FORWARD,icon_size=18, data=self.NEXT_HOUR, on_click=self._adjust_hh_min),
                    ft.Text(":"),
                ]),
                ft.Row([
                    ft.IconButton(icon=ft.icons.ARROW_BACK,icon_size=18, data=self.PREV_MINUTE, on_click=self._adjust_hh_min),
                    ft.Text(minute),
                    ft.IconButton(icon=ft.icons.ARROW_FORWARD, icon_size=18, data=self.NEXT_MINUTE, on_click=self._adjust_hh_min),
                ]),
            ], 
